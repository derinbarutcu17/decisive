...
app.whenReady().then(async () => {
  ...
  const script = [
      ...
      "  ok('task names control is available in Scatter view', !!namesSetting);",
      ...
      '  if (namesSetting) {',
      "    if (namesSetting.getAttribute('aria-pressed') === 'true') namesSetting.click();",
      "    namesSetting.click();",
      "    ok('task names control shows labels', namesSetting.getAttribute('aria-pressed') === 'true' && document.querySelector('#scatter-view').classList.contains('names-visible'));",
      "    namesSetting.click();",
      "    ok('task names control hides labels', namesSetting.getAttribute('aria-pressed') === 'false' && !document.querySelector('#scatter-view').classList.contains('names-visible'));",
      '  }',
      '  if (dots[0]) {',
