...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const script = [
      ...
      "  const namesToggle = document.querySelector('#task-names-toggle');",
      ...
      "  ok('task names toggle appears in Scatter view', namesToggle && !namesToggle.hidden);",
      ...
      '    namesToggle.click();',
      "    ok('task names toggle hides labels', namesToggle.getAttribute('aria-pressed') === initialNamesState && !document.querySelector('#scatter-view').classList.contains('names-visible'));",
      '  }',
      '  if (dots[0]) {',
